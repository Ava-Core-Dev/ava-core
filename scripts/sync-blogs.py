...
the websites. No Cursor required.
  ...
  python scripts/sync-blogs.py
  python scripts/sync-blogs.py --seed   # first time: copy existing posts into markdown
"""
...
AVA = Path(os.getenv("AVA_HOME", str(Path.home() / "Ava-Core")))
_MEDIA = AVA / "Media"
_POSTS_CANDIDATES = (
    _MEDIA / "public" / "documents" / "reports" / "posts",
    _MEDIA / "documents" / "reports" / "posts",
)
MEDIA_POSTS = next((p for p in _POSTS_CANDIDATES if p.is_dir()), _POSTS_CANDIDATES[0])
...
def main() -> int:
    ...
    print("Ava/Root Record/alexrs94 go live when GitHub auto-push runs (every 15 min) if Vercel is connected.")
