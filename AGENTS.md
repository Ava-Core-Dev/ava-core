...
Task Scheduler: `pythonw.exe` only — `windows\watchdog.py`, `scripts\auto-push.py` (15 min task), `scripts\auto-pull.py` (10 min, ff-only, refuses dirty), `scripts\site-update.py` (5 min, holding worker). Never `powershell.exe`.
