...
async def api_obs_solar_desk():
    ...
    try:
        ...
        # Windows psutil.getloadavg() starts at 0/0/0 — not a Unix load average.
        if os.name != "nt":
            try:
                load = psutil.getloadavg()
                extra["load_1"] = round(load[0], 2)
                extra["load_5"] = round(load[1], 2)
                extra["load_15"] = round(load[2], 2)
            except Exception:
                pass
        host = {**host, **extra}
        try:
            from apps.core.crons.since_last_fire.solar_weather import record_host_sample, _host_temp_c
...
async def api_obs_weather_desk():
    ...
    try:
        ...
        nws = latest_report("nws-weather-*.md")
        if nws:
            text = nws.read_text(encoding="utf-8", errors="replace")
            out["ts"] = datetime.fromtimestamp(
                nws.stat().st_mtime, tz=timezone.utc
            ).isoformat()
