...
async def api_goals(request: Request):
    if not config.PUBLIC_GOALS and not _allow_mutate(request):
        return JSONResponse({"error": "goals_hidden"}, status_code=404)
    return store.list_goals()


@router.get("/goals/{goal_id}")
async def api_goal(goal_id: str, request: Request):
    if not config.PUBLIC_GOALS and not _allow_mutate(request):
        return JSONResponse({"error": "goals_hidden"}, status_code=404)
