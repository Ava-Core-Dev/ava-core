...
async def vercel_webhook(request: Request):
    ...
    dep = dict(inner) if isinstance(inner, dict) else {}
    if isinstance(dep.get("deployment"), dict):
        nested = dep["deployment"]
        dep = {**dep, **nested}
