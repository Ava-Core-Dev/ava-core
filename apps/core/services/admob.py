...
TOKEN_JSON = Path(
    os.environ.get(
        ...
        str(_SECRETS / "google-admob-token.json"),
    )
)
SCOPES = [
...
DEFAULT_PUBLIC_REDIRECT = "http://127.0.0.1:8787/api/ops/admob/oauth/callback"
...
def status() -> dict[str, Any]:
    out: dict[str, Any] = {
        ...
        "note": (
            ...
            "Redirect URI: http://127.0.0.1:8787/api/ops/admob/oauth/callback "
            "(localhost only; do not publish /ops; do not use *.rootmc.net). Guide: docs/ADMOB-OAUTH.md"
        ),
