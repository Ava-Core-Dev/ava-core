...
class EarthquakeHawaiiPlugin(Plugin):
    ...
    def _make_voice(self, force: bool = False) -> Path | None:
        ...
        if not spoken:
            return None
        log.info("Hawaii EQ Ara script:\n%s", spoken)

        stamp = datetime.now(HST).strftime("%Y-%m-%dT%H")
        archive = config.GENERATED_DIR / f"earthquake-hawaii-{stamp}.mp3"
        current = config.GENERATED_DIR / "Earthquake_Hawaii_Current.mp3"
        try:
            self._tts(spoken, archive)
            import shutil
            shutil.copy2(archive, current)
            log.info("Saved Earthquake_Hawaii_Current.mp3")
            try:
