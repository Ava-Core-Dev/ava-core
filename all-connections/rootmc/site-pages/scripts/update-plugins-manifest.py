...
ROOTMC = Path("/home/ava-core/ava/workstations/rootmc")
WEB = ROOTMC / "Web Files" / "rootmc-web"
PUB = WEB / "public" / "plugins"
BUILD_PLUGINS = WEB / "build" / "plugins"
EMERG = ROOTMC / "emergent-repo" / "web" / "public" / "plugins"
TEST = Path("/home/ava-core/ava/workstations/minecraft-test/plugins")
HANDOFF_TEST = ROOTMC / "Server Handoffs" / "3. RootMC - Test Server" / "plugins"
UPLOAD = ROOTMC / "Server Handoffs" / "Upload Staging"
PLUGIN_LIBS = ROOTMC / "Plugin Building" / "Minecraft" / "plugins"
...
def load_env():
    ...
    for path in [
        ...
        Path("/home/ava-core/ava/workstations/rootmc/.env"),
    ]:
        if not path.exists():
            continue
...
def stage_newer_sources():
    ...
    for root in [
        HANDOFF_TEST,
        UPLOAD / "singular-latest" / "plugins",
        TEST,
        *sorted(PLUGIN_LIBS.glob("*/build/libs")),
    ]:
        if root.exists():
