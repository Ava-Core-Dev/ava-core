  ...
  function render(model) {
    ...
    var ledgerRows = [
      auditRow("Gold stored (/mint)", fmtGold(model.goldStoredG || model.mintNet),
        "legit map gold converted since opening", "/mint/"),
      ...
      auditRow("Player wallet Notes", fmtGold(model.walletNotes), "circulating G in Vault", "/balances/"),
      ...
      auditRow("Gold Backed Bond principal", fmtGold(model.bondPrincipal),
        model.bondHolders + " holders · " + fmtGold(model.bondUncollected) + " uncollected coupons", "/economy/bonds/"),
      auditRow("/mint ledger — gross in", fmtGold(model.mintGrossIn), "physical \u2192 Notes since 3 Jul", "/mint/"),
      auditRow("/mint ledger — redeemed", fmtGold(model.mintRedeemed), "Notes \u2192 physical (/mint gold)", "/mint/"),
      auditRow("/mint ledger — net backing", fmtGold(model.mintNet), "audited conversion total", "/mint/"),
      model.totalPayable > 0.01
        ? auditRow("Town + nation banks", fmtGold(model.townBankG + model.nationBankG), "Towny treasury balances", "/balances/")
        : "",
      model.goldBackedBondsG > 0.01
        ? auditRow("Personal Gold Backed Bonds", fmtGold(model.goldBackedBondsG), "paper /bonds — not double-counted with town auto-bonds", "/economy/bonds/")
        : "",
    ...
    var physicalRows = [
      ...
      auditRow("Unminted physical (inventory)", fmtGold(model.unmintedPhysical),
        "gold items at peg, not yet /mint", "/mint/"),
      ...
      auditRow("Gold-found tracker total", fmtGold(model.foundTotal),
        "all sources since 3 Jul", "/leaderboard/"),
    ];

    var bondRows = [
      auditRow("24h Gold Backed Bond pool", fmtGold(model.bondPool24h), "share of reserve inflows", "/economy/bonds/"),
      auditRow("Active Gold Backed Bond principal", fmtGold(model.bondPrincipal), "deposited via /bonds", "/economy/bonds/"),
    ];
