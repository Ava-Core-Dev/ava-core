(function () {
  ...
  var TREASURY = "/api/g2/rootmc/treasury/";
  /** Playtime + McMMO are shared across Gen1/Gen2 — always Gen1 API. */
  var SHARED_API = "/api/rootmc/server/";
  var FEATURED_FALLBACK_ID = "g2";
  var SHARED_FALLBACK_ID = "rootmc";
  var LIMIT = 50;
  ...
  async function resolveSharedServerId() {
    try {
      var res = await fetch("/api/rootmc/server/config", { cache: "no-store" });
      var data = await res.json().catch(function () { return {}; });
      var featured = data.featured_server || {};
      var sid = String(featured.server_id || "").trim();
      if (sid) return sid;
    } catch (e) { /* fall through */ }
    return SHARED_FALLBACK_ID;
  }
  ...
  async function load() {
    ...
    var base = API + encodeURIComponent(sid);
    var sharedId = await resolveSharedServerId();
    var sharedBase = SHARED_API + encodeURIComponent(sharedId);
    ...
    var boards = [
      ...
      { key: "playtime", path: sharedBase + "/playtime/leaderboard?limit=" + LIMIT, render: function (data) {
        renderPlaytime((data && data.leaderboard) || []);
        return data && data.synced_at;
      }},
      { key: "mcmmo", path: sharedBase + "/mcmmo/leaderboard?limit=" + LIMIT, render: function (data) {
