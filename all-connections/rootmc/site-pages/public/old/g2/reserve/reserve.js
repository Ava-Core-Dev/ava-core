(function () {
  ...
  function renderTownyIntake(mtd, allTime, townyChanges, data) {
    ...
    var period = monthPeriodLabel(data);
    var rows = [
      { key: "new_town", label: "New areas" },
      { key: "new_nation", label: "Extra areas" },
    ...
    if (!hasAny && !(Number((mtd || {}).other) || Number((allTime || {}).other))) {
      mtdHost.innerHTML =
        '<p class="rmc-muted">No tagged claim founding rows synced yet. New <code>/c</code> areas, expansions, and land fees log here after they hit the reserve ledger.</p>';
    ...
    if (otherMtd > 0 || otherAll > 0) {
      ...
      mtdHost.innerHTML +=
        ...
        '<div class="reserve-towny-label">Other land fees</div>' +
