(function () {
  ...
  var LEGACY_HOST_IDS = {
    "4963895e-0964-48b8-81b7-1f40a966e8be": true,
    "15bbc057-4f8b-4761-abdb-7b7e4d9c7512": true,
  };
  var LB_LIMIT = 8;
  ...
  async function resolveServerId() {
    ...
    try {
      ...
      if (sid && !LEGACY_HOST_IDS[sid.toLowerCase()]) return sid;
    } catch (_e) { /* fall through */ }
  ...
  function renderNoteSupply(reserve) {
    ...
    if (totalMeta) {
      totalMeta.textContent = payable
        ? "Wallets + claim banks + bond paper"
    ...
    if (payable) {
      dlRows.push(
        ...
        dlRow("Gold circulating", fmtGold(Number(payable.gold_circulating_g || payable.private_claims_g || payable.total_payable_g)), "wallets, claim banks, bond paper"),
        dlRow("Player wallets", fmtGold(Number(payable.player_wallet_g)), "spendable G"),
        dlRow("Claim banks", fmtGold(Number(payable.town_bank_g || payable.claim_bank_g)), "area banks for expansions"),
        dlRow("Trusted pools", fmtGold(Number(payable.nation_bank_g)), "legacy group balances if any remain"),
        ...
        Number(payable.reserve_vault_g) > 0.01 || Number(payable.reserve_vault_g) === 0
          ? dlRow("Server Reserve vault Notes", fmtGold(Number(payable.reserve_vault_g)), "custodial G — should track managed residual")
          : "",
        missingAmt > 0.01
  ...
  function scanDetailLabel(row) {
    ...
    if (Number(row.towny_placed_g) > 0.01) parts.push("placed " + fmtGold(row.towny_placed_g));
  ...
  async function load() {
    try {
      ...
      // Explicit ?server= (or /all-servers/<id>/) scopes treasury to that host; live hub stays on rootmc aggregate.
