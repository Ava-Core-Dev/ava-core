#!/usr/bin/env python3
"""Backfill June HST vote payouts into root_treasury_ledger from root_rewards_votes.

Inserts VOTE outflow rows (reserve → player), syncs to D1, and sets towny-server
balance to ledger net.
"""
import argparse
...
# HST month 2026-06 (UTC bounds used by rootmc-treasury monthBoundsIso)
JUNE_HST_START = "2026-06-01 10:00:00"
JUNE_HST_END = "2026-07-01 10:00:00"
BACKFILL_PREFIX = "backfill:vote:"
...
def build_entries(conn) -> list[tuple[float, str, str, str, str]]:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, uuid, service, voted_at, gold_earned "
        "FROM root_rewards_votes "
        "WHERE gold_earned > 0 AND voted_at >= %s AND voted_at < %s "
        "ORDER BY voted_at ASC, id ASC",
        (JUNE_HST_START, JUNE_HST_END),
    )
...
def main():
    ...
    parser.add_argument("--force", action="store_true", help="Delete existing vote backfill rows and re-insert")
    ...
    try:
        ...
        print(f"Planned {len(entries)} VOTE rows for HST 2026-06, total outflow {total} G")

        if backfill_exists_mysql(conn):
            if not args.force:
                print("Vote backfill rows already exist. Use --force to replace.")
            ...
            if not args.dry_run:
                ...
                print("Removed prior vote backfill rows.")
        ...
        print("June vote ledger backfill complete.")
    finally:
        conn.close()
