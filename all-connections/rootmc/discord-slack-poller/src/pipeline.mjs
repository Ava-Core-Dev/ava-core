...
function isPropGovernanceSurface(channelId = "", channelName = "") {
  ...
  if (["1537333521389977600", "1522406451413385317", "1522413185364398090"].includes(id)) return true;
...
export function createPipeline(deps) {
  ...
  async function handleTrigger(channelId, msg, messages = [], overrides = {}) {
    ...
    try {
      ...
      const propSurf = isPropGovernanceSurface(channelId, channel?.name || "");
  ...
  async function ingestMessage(msg, { messages = [], isDm = false } = {}) {
    ...
    if (isLockoutActive()) {
      ...
      // /deep — full/delta channel dump summary to Alex TG
      if (isDeepCommand(msg.content || "")) {
        const deep = await tryHandleDeepCommand({
          text: msg.content || "",
          authorId: msg.author?.id,
          isDm: true,
          surface: surfEarly,
        ...
        if (deep?.handled && deep.reply) {
          touchActivity("deep-dump");
          try {
            await reply(channelId, deep.reply, msg.id);
          } catch {
            /* ignore */
          }
          return;
    ...
    // /deep — Alex private dump of all chats since last dump
    if (isDeepCommand(msg.content || "")) {
      const deep = await tryHandleDeepCommand({
        text: msg.content || "",
        authorId: msg.author?.id,
        isDm:
          ...
          (msg.surface === "telegram" &&
            msg.telegram?.chatType === "private"),
        surface: msg.surface || pipelineSurface(msg, channelId),
        channelId,
      });
      if (deep?.handled && deep.reply) {
        touchActivity("deep-dump");
