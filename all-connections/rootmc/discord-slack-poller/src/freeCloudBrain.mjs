  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        { role: "user", content: String(user || "").slice(0, 28000) },
      ],
    }),
...
async function geminiChat({ key, model, system, user }) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      systemInstruction: { parts: [{ text: String(system || "").slice(0, 24000) }] },
      contents: [
        {
          role: "user",
          parts: [{ text: String(user || "").slice(0, 28000) }],
        },
      ],
  ...
  switch (id) {
    case "groq":
      return openAiCompatChat({
        ...
        key: envTrim("GROQ_API_KEY"),
        model: envTrim("GROQ_MODEL") || "llama-3.1-8b-instant",
        system,
        user,
      });
    case "gemini":
