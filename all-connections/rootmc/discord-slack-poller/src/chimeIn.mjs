...
function isNoChimeChannel(channelId) {
  ...
  if (id === AVA_CHANNELS.randomFacts || id === "1545287379055812638") return true;
  return false;
}
...
export function shouldAvaChimeIn({
  ...
  const homeId = String(AVA_CHANNELS.avaHome || "");
  const discordOffHome =
    /^\d{17,20}$/.test(ch) &&
    !isSlackChannelId(ch) &&
    ch &&
    homeId &&
    ch !== homeId;
  if (discordOffHome && !(helpQ && rootmc)) {
    return { chime: false, reason: "off_home" };
  }
