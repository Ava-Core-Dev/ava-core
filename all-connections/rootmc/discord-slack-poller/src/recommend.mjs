...
export function isWakeCommand(content) {
  const q = String(content || "")
    ...
    .replace(/\s+/g, " ")
    .trim();
  // Affection "come back to me" is companion talk — not hush wake
...
export async function recommend({
  ...
  const premiumCursor = isQuietOperator(authorId);
  if (!q) {
    return scrubOut(
    ...
    if (dream.ok && dream.text) {
      try {
        ...
        recordLocalLesson({
          question: q,
          answer: dream.text,
          teacher: dream.via === "free_cloud" ? "free_cloud" : "dream",
          surface: surfaceNorm,
          authorId,
          coreOnline: true,
          meta: {
            provider: dream.provider || null,
            via: dream.via || "xai",
            path: "mode-grok",
            authorName,
          },
  ...
  if (sessionMode === "cursor" && cursorUp && !forceDream && !asleep) {
    ...
    if (looksLikeDigUsageOutage(dream.reason)) {
      markDigOutage(dream.reason, { source: "dream" });
      // Survive on free cloud / llama instead of heuristic dark spam
      try {
        const { freeCloudConfigured, freeCloudChat } = await import(
          "./freeCloudBrain.mjs"
        );
        if (freeCloudConfigured()) {
          const free = await freeCloudChat({
            system:
              "You are Ava Ivy. Warm, short reply. Never name AI vendors on public Discord.",
            user: `Ask:\n${q}\n\nContext:\n${String(context || "").slice(0, 2000)}`,
            surface: surfaceNorm,
          });
          if (free.ok && free.text) {
            const { recordLocalLesson } = await import("./localBrain.mjs");
            const { scrubPublicReply } = await import("./scrub.mjs");
            ...
            recordLocalLesson({
              ...
              authorId,
              coreOnline: true,
              meta: { provider: free.provider, rescue: "dream_outage" },
            });
            return text;
          }
        }
        ...
        console.warn("dream→free/llama rescue:", err?.message || err);
      }
    }
