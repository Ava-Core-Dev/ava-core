...
const DEFAULTS = {
  ...
  reserves: {
    claims: { gold: 0, pausedPayouts: false, isolated: true },
    towny: { gold: 0, pausedPayouts: false, isolated: true },
  },
...
export function tokenBoardText() {
  ...
  lines.push(
    "",
    `Reserve Claims: ${r.claims.gold} G paused=${r.claims.pausedPayouts}`,
    `Reserve Towny: ${r.towny.gold} G paused=${r.towny.pausedPayouts}`,
