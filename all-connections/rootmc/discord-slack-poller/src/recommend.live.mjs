...
export async function recommend({
  ...
  if (sessionMode === "cursor" && cursorUp && !forceDream && !asleep) {
    ...
    const dreamReason = String(dream.reason || "");
    if (looksLikeDigUsageOutage(dreamReason)) {
      markDigOutage(dreamReason, { source: "dream" });
    }
    // Survive on llama core whenever dream fails (missing key / 402 / timeout)
    try {
      if (await shouldUseLocalBrain(env || {})) {
        const local = await localRecommend({
          question: q,
    ...
    // No Grok / credit outage: fall through to Root Server dig when Cursor key exists
    // instead of ghosting Discord with heuristics-only.
    const dreamOutage =
      /^http_40[23]$/i.test(dreamReason) ||
      /missing_dream_key/i.test(dreamReason) ||
      looksLikeDigUsageOutage(dreamReason);
    ...
    if (canCursorFailover) {
      console.warn("Ava dream outage → Cursor failover", dreamReason);
    }
  }
