...
/** Send a file attachment on Telegram as Ava. */
export async function postAvaTelegramDocument({
  chatId,
  filePath,
  caption = "",
  replyToMessageId = null,
  ...
  const channelId = isTelegramChannelId(chatId)
    ? String(chatId)
    : toTelegramChannelId(chatId);
  const cleanedCaption = scrubPublicReply(caption, { surface: "telegram" });
  try {
    ...
    recordAvaUtterance({
      surface: "telegram",
      channelId,
      content: cleanedCaption || `[file:${filePath}]`,
      refId: replyToMessageId ? String(replyToMessageId) : null,
      kind,
      source,
      ok: true,
    ...
    return { id: String(sent?.message_id || ""), ...sent };
  } catch (err) {
    recordAvaUtterance({
      surface: "telegram",
      channelId,
