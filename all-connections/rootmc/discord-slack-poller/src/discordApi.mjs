...
export async function pinMessage(fetchJson, channelId, messageId) {
  return fetchJson(`/channels/${channelId}/pins/${messageId}`, {
    method: "PUT",
  });
}
...
export async function forwardMessage(
  ...
  return fetchJson(`/channels/${destChannelId}/messages`, {
    method: "POST",
    body: JSON.stringify({
      message_reference: {
        type: 1,
        message_id: String(messageId),
        channel_id: String(sourceChannelId),
      },
      allowed_mentions: { parse: [] },
    }),
  });
