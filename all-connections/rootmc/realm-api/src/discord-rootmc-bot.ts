...
function helpText(): string {
  return (
    ...
    "- **`/server`**  -  now on **Ava** (use Ava slash)\n" +
  ...
  if (t === 2) {
    ...
    if (
      ...
      ctx!.waitUntil(
        (async () => {
          try {
            if (cmd === "server") { /* MOVED_TO_AVA_SERVER */
              await patch("**/server** moved to **Ava** — use Ava's `/server` (Official RootMC bot is phasing this out).");
              return;
            }
            if (cmd === "vote") {
              const userId = interactionDiscordUserId(body);
    ...
    if (cmd === "server") { /* MOVED_TO_AVA_SERVER */
      const content = "**/server** moved to **Ava** — use Ava's `/server` (Official RootMC bot is phasing this out).";
      return interactionJson(4, { content, flags: 64 });
    }
