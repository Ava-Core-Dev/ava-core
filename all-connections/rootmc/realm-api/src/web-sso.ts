...
export function allowedWebCredentialOrigin(origin: string | null): string | null {
  ...
  try {
    ...
    if (host === "rootmc.net" || host === "www.rootmc.net") return o;
    if (host.endsWith(".rootmc.net")) return o;
...
export function ssoCookieDomainForApiHost(apiHost: string): string | null {
  ...
  if (h === "rootmc.net") return null;
  if (h.endsWith(".rootmc.net")) return ".rootmc.net";
  return null;
}
