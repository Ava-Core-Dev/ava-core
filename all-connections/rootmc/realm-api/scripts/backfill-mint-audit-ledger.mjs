/**
 * Import /mint audit rows (mint:gross= / mint:redeem=, amount 0) from MySQL into D1.
 * These were skipped by sync while amount <= 0 was filtered out.
 *
 *   node scripts/backfill-mint-audit-ledger.mjs
 *   node scripts/backfill-mint-audit-ledger.mjs --dry-run
 */
...
const [rows] = await conn.execute(
  `SELECT id, entry_type, amount, from_uuid, to_uuid, details, created_at
   FROM ${table}
   WHERE created_at >= ?
     AND (details LIKE 'mint:gross=%' OR details LIKE 'mint:redeem=%'
          OR details LIKE 'correction:tax_reclass:mint:gross=%'
          OR details LIKE 'correction:tax_reclass:mint:redeem=%')
   ORDER BY id ASC`,
  [POST_RESET_START],
);
await conn.end();

console.log("MySQL /mint audit rows since opening:", rows.length);
if (dryRun) {
  console.log("sample:", rows.slice(-10));
...
console.log("Upserted", upserted, "/mint audit rows into D1.");
