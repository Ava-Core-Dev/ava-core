-- Guild channel registry (GET /guilds/:id/channels) + per-message stats for charts (not the mobile developer feed).
-- Replaces discord_activity_counted_message idempotency: one row per Discord message snowflake globally.
DROP TABLE IF EXISTS discord_activity_counted_message;
