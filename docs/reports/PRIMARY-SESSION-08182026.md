...
Vercel: the standalone `ava-ivy-cloud` and `rootrecord-cloud` repos are the deployable Next.js apps. Import each repo as its own Vercel project, set `AVA_ORIGIN_URL` in project environment variables, and attach `avaivy.cloud` and `rootrecord.cloud` respectively only after green builds and public API checks. The old Worker remains the rollback path until that cutover is verified.
...
1. **Vercel:** Import `ava-ivy-cloud` and `rootrecord-cloud` as separate projects. Set `AVA_ORIGIN_URL`, verify the public allowlisted handlers, then point `avaivy.cloud` and `rootrecord.cloud` at Vercel. Retain the Worker until rollback is no longer needed.
