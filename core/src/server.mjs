...
const server = http.createServer(async (req, res) => {
  ...
  // AI core context — HTML / markdown / JSON
  if (
    (method === "GET" || method === "HEAD") &&
    (url.pathname === "/context" ||
      url.pathname === "/status/context" ||
      url.pathname === "/ava/context" ||
      url.pathname === "/context.md" ||
      url.pathname === "/status/context.md" ||
      url.pathname === "/ava/context.md")
  ) {
    ...
    if (wantJson) {
      ...
      res.end(JSON.stringify(avaContextPayload(), null, 2));
      return;
    }
    if (wantMd) {
    ...
    const basePath =
      fwd ||
      (url.pathname.startsWith("/status/context")
        ? "/status"
        : url.pathname.startsWith("/ava/context")
          ? "/ava/status"
          : "");
    res.end(avaContextPageHtml({ basePath }));
    return;
  }

  if ((method === "GET" || method === "HEAD") && url.pathname === "/api/context") {
    res.setHeader("Content-Type", "application/json; charset=utf-8");
    res.setHeader("Access-Control-Allow-Origin", "*");
    res.setHeader("Cache-Control", "public, max-age=60");
    if (method === "HEAD") {
