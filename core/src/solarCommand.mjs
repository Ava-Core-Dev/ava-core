 ...
 * Discord: text `/solar` in ANY RootMC guild channel (+ slash). Watch allowlist does not apply.
...
export function isSolarCommand(text = "") {
  ...
  // /status + plain "status report" — same live board (never LLM-paraphrase EcoFlow names)
  if (/^\/status(?:\s|$)/i.test(t)) return true;
  if (/^status(?:\s+report)?$/i.test(t)) return true;
  if (/^status report(?:\s+pls|\s+please)?[.!?]?$/i.test(t)) return true;
  return false;
}
 ...
 */
export async function buildSolarCommandReply({ refreshPower = true, commandLabel = "solar" } = {}) {
  ...
  const lines = [
    `**${site.label || "HI Pacific Solar Root Server"}** — \`/${commandLabel === "status" ? "status" : "solar"}\` @ ~${fmtHstClock()}${ageNote}`,
    "",
...
export async function tryHandleSolarCommand({ text = "", isAlex = false } = {}) {
  ...
  const t = String(text || "").trim();
  const commandLabel = /^\/status\b|^status\b/i.test(t) ? "status" : "solar";
  const reply = await buildSolarCommandReply({ refreshPower: true, commandLabel });
