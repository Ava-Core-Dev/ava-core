...
export function rconTargets() {
  ...
  if (shared) {
    ...
    // Dual Claims/Towny RCON retired — opt-in only
    const allowLegacy = String(process.env.AVA_RCON_ALLOW_LEGACY_HOSTS || "").trim() === "1";
    if (allowLegacy) {
...
function defaultTargetId() {
  ...
  if (!targets.length) return pref || "primary";
