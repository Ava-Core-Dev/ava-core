...
const ROOT_RESTART_RE =
  /(?:^|[\s/])(?:ava[,:]?\s+)?(?:please\s+)?(?:run\s+)?\/?rootrestart(?:\s+(cancel|primary|test))?(?:\s+(cancel|primary|test))?/i;
...
function parseRootRestartArgs(content) {
  ...
  else if (/\bclaims\b/.test(q)) target = "primary";
...
export async function askPlayersRootRestart({ target = "primary" } = {}) {
  const msg =
