  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .about-ava h2 {
      ...
      color: var(--lime); text-shadow: 0 0 18px rgba(184,255,92,0.22);
    }
    ...
    .links a.signin:hover { border-color: rgba(110, 231, 168, 0.55); }
    .links details.ops {
      position: relative;
    }
    ...
    .full-totals h2 {
      ...
      color: var(--lime); text-shadow: 0 0 18px rgba(184,255,92,0.18);
    }
    ...
    table.totals tr.section td {
      ...
      text-transform: uppercase; letter-spacing: 0.05em; border-bottom-color: rgba(110,231,168,0.25);
    }
...
<body>
  <main>
    ...
    <section class="about-ava" id="aboutAva" aria-label="What is Ava Ivy">
      ...
      <p><strong>Ava Ivy</strong> is lead developer for the Root Record Ecosystem — not a help-desk bot.</p>
      <p>She plans, ships, and watches live systems from this Root Server home. Player features go through votes; bugs get verified, then fixed. In-game money stays <strong>Gold</strong>; real support stays on the Pro pages. Part runtime, part character — built to keep the gaming servers advancing with less babysitting over time.</p>
    </section>
  ...
  <script>
    ...
    function render(d) {
      ...
      $("banner").innerHTML =
        statusPill(live) +
        (mineOnline
          ? '<span class="pill ok">Gaming boost '+esc(mineMult)+'</span>'
          : '<span class="pill bad">Gaming boost offline 1.0×</span>') +
      ...
      $("kpis").innerHTML = [
        ...
        ["Gaming boost", mineMult, mineHint, mineOnline],
      ...
      $("stats").innerHTML = [
        ...
        ["Bank / load", [
          ...
          ["Gaming boost", mineOnline ? mineMult + " · 1+(SOC/100)" : "1.0× (offline)"],
      ...
      (function renderTotals() {
        ...
        body.innerHTML = [
          ...
          section("Bank / mining"),
          line("Bank avg", (w) => w.bank?.avgPct, (v) => fmt(v, "%")),
          line("Bank min", (w) => w.bank?.minPct, (v) => fmt(v, "%")),
          line("Bank max", (w) => w.bank?.maxPct, (v) => fmt(v, "%")),
    ...
    async function tick() {
      try {
        const res = await fetch("${api("/api/solar")}?hours=8", { cache: "no-store" });
        if (!res.ok) {
