...
export function scrubPublicReply(text, opts = {}) {
  ...
  const maxPublic = Number(opts.maxChars) || (opts.allowLongProp ? 1900 : 450);
  if (!opts.allowLongProp && out.length > maxPublic) {
  ...
  if (surface === "telegram" || surface === "discord-dm") {
    max = Number(process.env.AVA_TG_REPLY_MAX || 1800) || 1800;
  }
