...
export function loadEcoMinuteSeries(opts = {}) {
  ...
  const limit = Math.max(12, Math.min(45000, Number(opts.limit ?? 240)));
  const since = Date.now() - maxAgeMs;
