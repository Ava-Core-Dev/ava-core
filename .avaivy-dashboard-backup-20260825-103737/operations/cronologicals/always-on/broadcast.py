...
def uptime_report():
    """Compute uptime availability while explicitly counting inferred offline gaps.

    A gap between samples is offline when the uptime counter resets or stops advancing.
    A sample immediately before a reporting window is retained as an anchor so an
    overnight shutdown that began before the window is not silently discarded.
    """
    ...
    for row in rows:
        try:
            ...
            events.append((ts,float(row["current_uptime_seconds"])))
        except Exception:
            pass
    now=datetime.now(timezone.utc).timestamp()
    report={}
    for name,seconds in WINDOW_SECONDS.items():
        cutoff=0 if seconds is None else now-seconds
        prior=[e for e in events if e[0] < cutoff]
        selected=[e for e in events if e[0] >= cutoff]
        anchor=prior[-1] if prior else None
        series=([anchor] if anchor else []) + selected

        elapsed=observed=offline=0.0
        # Start measuring at the actual window boundary when possible.
        if not selected:
            elapsed=max(0.0,now-cutoff)
            offline=elapsed
        elif series:
            prev_t,prev_u=series[0]
            if prev_t < cutoff:
                prev_t=cutoff
            elif prev_t > cutoff:
                prev_t=cutoff
                prev_u=None
            for cur_t,cur_u in series[1:]:
                span=max(0.0,cur_t-prev_t)
                elapsed += span
                if prev_u is not None and cur_u >= prev_u:
                    online_span=min(span, max(0.0,cur_u-prev_u))
                    observed += online_span
                    offline += max(0.0, span-online_span)
                else:
                    offline += span
                prev_t,prev_u=cur_t,cur_u
            # If the most recent sample is still inside the window, count time to now
            # as online only when its uptime is current enough to cover that span.
            if selected:
                last_t,last_u=selected[-1]
                span=max(0.0,now-last_t)
                if span <= 180:
                    elapsed += span
                    observed += span
                else:
                    # Collector silence beyond the tolerance is an inferred offline gap.
                    elapsed += span
                    offline += span
        ...
        report[name]={
            ...
            "current_uptime_seconds":vals[-1] if vals else None,
            "average_uptime_seconds":sum(vals)/len(vals) if vals else None,
            "observed_seconds":observed,
            "offline_seconds":offline,
            "window_seconds":elapsed,
            "availability_percent":avail,
            "online_percent":avail,
            "offline_percent":(100-avail) if avail is not None else None,
            "percent_change":((vals[-1]-vals[0])/abs(vals[0])*100) if len(vals)>1 and vals[0] else None,
...
def uptime_history(window="all"):
    ...
    prev=None
    for ts,up in events:
        gap=(ts-prev[0]) if prev else 0
        reset=(prev is not None and up < prev[1])
        online=(prev is None or (gap <= 180 and not reset))
        out.append({"ts":ts,"uptime_seconds":up,"online":online,"gap_seconds":gap,"reset":reset})
        prev=(ts,up)
    return out

def operations_status():
    """Local scheduler inventory, forecast and current process snapshot for status."""
    ...
    forecast=_schedule_forecast(3600)
    return {"pending_crons": pending, "processes": processes, "forecast": forecast, "averages": _schedule_averages()}
...
def geography_earthquakes(country="", state="", location=""):
    """Return global seismic events, preferring the canonical DB with a legacy fallback."""
    matches=_geo_match(country,state,location)
    ...
    if GLOBAL_EARTHQUAKES_DB.exists():
        ...
        try:
            if ids:
                ...
                rows=con.execute(f"SELECT * FROM earthquakes WHERE location_id IN ({qmarks}) ORDER BY observed_at DESC LIMIT 100",ids).fetchall()
            elif country:
                ...
                rows=con.execute("SELECT * FROM earthquakes ORDER BY observed_at DESC LIMIT 100").fetchall()
        finally:
            con.close()
    # The 5-minute collector historically wrote database/quakes.db. Use it so
    # the public page never becomes empty merely because the canonical migration
    # database has not yet been populated.
    if not rows and LEGACY_QUAKES_DB.exists():
        con=sqlite3.connect(str(LEGACY_QUAKES_DB)); con.row_factory=sqlite3.Row
        try:
            sql="SELECT id,time_utc,mag,depth_km,place,latitude,longitude,url FROM quakes WHERE source='global' ORDER BY time_ms DESC LIMIT 100"
            rows=con.execute(sql).fetchall()
        finally:
            con.close()
        events=[{
            "event_id":r["id"],"observed_at":r["time_utc"],"magnitude":r["mag"],
            "depth_km":r["depth_km"],"place":r["place"],"lat":r["latitude"],
            "lon":r["longitude"],"source_url":r["url"],"country_code":None,
            "admin1_code":None,"location_id":None
        } for r in rows]
    else:
        events=[dict(r) for r in rows]
    return {"ok":True,"events":events,"locations":matches}
...
class H(BaseHTTPRequestHandler):
    ...
    def api(self, path, parsed):
        ...
        if path in ("/api/uptime", "/uptime/api"):
            window=_window_from_query(parsed, "24h")
            self.js({"ts": datetime.now(timezone.utc).isoformat(), "window":window, "periods":uptime_report(), "history":uptime_history(window)})
            return True
