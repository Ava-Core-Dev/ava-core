...
function chart(id,sets,labels){

 const svg=document.getElementById(id);
 const w=1000,h=300;
 const pL=50,pR=18,pT=18,pB=55;

 const values=sets.flatMap(s=>s.values).filter(Number.isFinite);

 if(!values.length){
  svg.innerHTML=
   '<text x="500" y="150" text-anchor="middle" fill="#93a0b5">No data in selected window</text>';
  return;
 }
 ...
 const x=i=>
  pL+(n<2?(w-pL-pR)/2:i*(w-pL-pR)/(n-1));

 const y=v=>
  h-pB-(v-lo)/(hi-lo)*(h-pT-pB);
 ...
 for(let j=0;j<=4;j++){
  ...
  html+=
   ...
   `<text x="${pL-7}" y="${yy+4}" text-anchor="end"
      fill="#93a0b5" font-size="11">${fmt(v,0)}</text>`;
 }
 ...
 for(let j=0;j<=4;j++){
  ...
  html+=
   `<line x1="${xx}" y1="${h-pB}" x2="${xx}" y2="${h-pB+5}"/>`+
   `<text x="${xx}" y="${h-15}" text-anchor="middle"
      fill="#93a0b5" font-size="11">${labels[i]||''}</text>`;
 }
